...
var table;
var baseUrlShow = "{{ route('admin.cashflow.units.show', ['cashFlowUnit' => 'tempId']) }}";
var baseUrlEdit = "{{ route('admin.cashflow.units.edit', ['cashFlowUnit' => 'tempId']) }}";
var baseUrlDelete = "{{ route('admin.cashflow.units.destroy', ['cashFlowUnit' => 'tempId']) }}";
...
function activartabla() {
    ...
    table = $('#datatable').DataTable({
        ...
        buttons: [
            ...
            @permission('cashflow-unit-delete')
            {
        ...
        /*============================AJAX============================*/
        ajax: "{{ route('admin.cashflow.units.indexDatatable') }}",
        /*============================AJAX============================*/
...
function ModalEdit(uuid) {
    ...
    $.get(url, function(data) {
        ...
        $('#uuid_edit').val(data.unit.uuid);
        //asignamos los valores a los inputs
        $('#name_edit').val(data.unit.name);
        $('#description_edit').val(data.unit.description);

        // Elimina las opciones de categoría existentes
        $('#category_id_edit').empty();

        // Añade una opción por defecto
        $('#category_id_edit').append('<option value="">Seleccione un tipo de unidad</option>');

        $('.unit-form').hide(); // Oculta todos los formularios

        // Itera sobre las categorías
        $.each(data.categories, function(index, category) {
            // Crea una nueva opción para la categoría
            var option = $('<option value="' + category.id + '" data-type-name="' + category.name + '">' + category.name + '</option>');

            // Si la categoría es la misma que la del CashFlowUnit, selecciona la opción
            if (category.id === data.unit.category_id) {
                option.attr('selected', 'selected');
            }

            // Añade la opción al select de categorías
            $('#category_id_edit').append(option);
        });


       // Elimina los campos de entrada de atributos existentes
       $('#attributes_container').empty();

        // Parsea los atributos a un objeto
        var attributes = JSON.parse(data.unit.attributes);

        // Itera sobre los atributos
        for (var attribute in attributes) {
            // Verifica si el atributo no es nulo
            if (attributes[attribute] !== null) {
                // Si el atributo no es nulo, crea un nuevo campo de entrada para el atributo
                var input = $('<div class="form-group attribute-input">' +
                    '<label for="edit_' + attribute + '">' + attribute + '</label>' +
                    '<input type="text" class="form-control attribute-input" id="edit_' + attribute + '" name="attributes[' + attribute + ']" value="' + attributes[attribute] + '">'+
                    '</div>');
                // Añade el nuevo campo de entrada al contenedor de atributos
                $('#attributes_container').append(input);
            }
        }
...
function ModalShow(uuid) {
    ...
    $.get(url, function(data) {
        ...
        // Limpia el contenedor de atributos
        $('#attributes_show').empty();
        // Parsea los atributos a un objeto
        ...
        for (var attribute in attributes) {
            // Verifica si el atributo no es nulo
            if (attributes[attribute] !== null) {
                // Si el atributo no es nulo, muestra el atributo en el modal
                var attributeElement = $('<input type="text" readonly class="form-control" value="' + attribute + ': ' + attributes[attribute] + '">');
                $('#attributes_show').append(attributeElement);
            }
        }
