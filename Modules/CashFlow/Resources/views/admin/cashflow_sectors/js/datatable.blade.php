...
var table;
var baseUrlShow = "{{ route('admin.cashflow.sectors.show', ['cashFlowSector' => 'tempId']) }}";
var baseUrlEdit = "{{ route('admin.cashflow.sectors.edit', ['cashFlowSector' => 'tempId']) }}";
var baseUrlDelete = "{{ route('admin.cashflow.sectors.destroy', ['cashFlowSector' => 'tempId']) }}";
var getSubSectorsUrl = "{{ route('admin.cashflow.sectors.getSubSectors', ['cashFlowSector' => ':tempId']) }}";
...
function activartabla() {
    ...
    table = $('#datatable').DataTable({
        ...
        buttons: [
            ...
            @permission('cashflow-subcategory-delete')
            {
        ...
        /*============================AJAX============================*/
        ajax: "{{ route('admin.cashflow.sectors.indexDatatable') }}",
        /*============================AJAX============================*/
        ...
        columns: [
            ...
            //action
            {
                ...
                render: function(data, type, row) {
                    ...
                    @permission('cashflow-subsector-create')
                    buttons += `
                    <span data-placement="top" data-toggle="tooltip" title="Agregar Subcategoría">
                        <button type="button" class="btn btn-success" onclick="ModalcreateSubsector('${data}')">
                            <i class="fas fa-plus"></i>
                        </button>
                    </span>`;
                    @endpermission

                    @permission('cashflow-sector-view')
                    buttons += `
                    ...
                    @permission('cashflow-sector-edit')
                    buttons += `
                    ...
                    @permission('cashflow-sector-delete')
                    buttons += `
                    ...
                    @permission('cashflow-subsector-view')
                    buttons += `
...
$('#datatable').on('click', '.details-control', function() {
    ...
    if (row.child.isShown()) {
        $(`#subsectors-datatable-${uuid}`).DataTable().destroy();
        row.child.hide();
        tr.removeClass('shown');
    } else {
        var url = getSubSectorsUrl.replace(':tempId', uuid);
        $.get(url, function(subcategorias) {
            var childRowId = `subsectors-datatable-${uuid}`;
            row.child(`<div><table class="table subsectors-datatable" id="${childRowId}"></table></div>`).show();
            tr.addClass('shown');
        ...
        columns: [
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    @permission('cashflow-subsector-edit')
                    buttons += `<button type="button" class="btn btn-info" onclick="ModaleditSubsector('${data.uuid}')">Editar</button>`;
                    @endpermission

                    @permission('cashflow-subsector-delete')
                    buttons += `<button type="button" class="btn btn-danger" onclick="ModaldeleteSubsector('${data.uuid}')">Eliminar</button>`;
                    @endpermission
...
function ModalcreateSubsector(uuid) {
    //asignamos el uuid de la categoria padre
    $('#uuid_sector').val(uuid);
    //abrimos modal subcategoria
    $('#createSubsector').modal('show');
}

function ModaleditSubsector(uuid) {
    // Actualiza los elementos del modal con la información de la categoría

    var baseUrleditSubsector = "{{ route('admin.cashflow.subsector.edit', ['cashFlowSubSector' => 'tempId']) }}";
    var url = baseUrleditSubsector.replace('tempId', uuid);
    url = url.replace('tempId', uuid);
    $.get(url, function(data) {
        //guardamos el uuid en mi input hidden
        $('#uuid_edit_subsector').val(data.uuid);
        //asignamos los valores a los inputs
        $('#name_edit_subsector').val(data.name);
        $('#description_edit_subsector').val(data.description);
    });

    // Muestra el modal
    $('#editSubsector').modal('show');
}

function ModaldeleteSubsector(uuid) {
   //asignamos el uuid de la subcategoria
    $('#uuid_delete_subsector').val(uuid);

    //abrimos modal subcategoria
    $('#deleteSubsector').modal('show');
}
</script>
