columns: [
   ...
   { data: null, targets:0, orderable:false,  render: function ( data, type, row ) {
       ...
       <span data-placement="top" data-toggle="tooltip" title="EDITAR" >
           <button type="button" class="btn btn-dark" onclick="ModalEditar(`+"'"+data.uuid+"'"+`);"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
       </span>
         @endpermission
   
       @permission('user-delete')
       <span data-placement="top" data-toggle="tooltip" title="ELIMINAR" >
           <button type="button" class="btn btn-danger" onclick="ModalDelete(`+"'"+data.uuid+"'"+`);"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
       </span>
        @endpermission
                     `}
